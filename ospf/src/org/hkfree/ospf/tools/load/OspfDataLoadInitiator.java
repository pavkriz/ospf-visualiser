...
public class OspfDataLoadInitiator {
	...
	private void loadTopologyDataFromTelnet(OspfModel ospfModel, TelnetClient tc) throws IOException, InterruptedException {
		((OspfWin) winManager.getOwner()).getStateDialog().addText("Stahuji data topologie...");
		...
		OspfLoader dataLoader = new OspfLoader();
		((OspfWin) winManager.getOwner()).getStateDialog().operationFinished();
		((OspfWin) winManager.getOwner()).getStateDialog().addText("Nacitam data topologie...");
	...
	private void loadNonTopologyDataFromTelnet(OspfModel ospfModel, TelnetClient tc) throws IOException,
		...
		((OspfWin) winManager.getOwner()).getStateDialog().addText("Stahuji netopologicka data...");
		...
		// nazev
		((OspfWin) winManager.getOwner()).getStateDialog().operationFinished();
		((OspfWin) winManager.getOwner()).getStateDialog().addText("Nactim nazvy routerů přes rDNS...");
		...
		((OspfWin) winManager.getOwner()).getStateDialog().operationFinished();
		((OspfWin) winManager.getOwner()).getStateDialog().close();
	}
