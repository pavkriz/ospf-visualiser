...
public class OspfWinManager {
    ...
    private MODE mode0 = MODE.TRANSFORMING; // akutalni mody mapy
	...
	for (String sourcePath : settings.getFilePaths()) {
	    ...
	    OspfModel actualModel = ospfModely.get(ospfModely.size() - 1);
	    try {
		...
		((OspfWin) owner).addAndFillModelTabbedPane(actualModel.getModelName(), actualModel);
	    } else {
		ospfModely.remove(actualModel);
	    }
	}
	checkActionsEnable();
     ...
     */
    protected void openLoadLoadSelection() {
